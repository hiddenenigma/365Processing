...
class Flower {
  ...
  Flower(int n, float d, Vec2D center) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      nodes.add(new Node(center.add(Vec2D.randomVector()), random(3,15)));
    }
  ...
  void showConnections(Flower other) {
    stroke(0,20);
    ArrayList otherNodes = other.getNodes();
    for (int i = 0; i < nodes.size(); i++) {
      VerletParticle2D pi = (VerletParticle2D) nodes.get(i);
