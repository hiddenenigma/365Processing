...
void setup() {
  ...
  for (int i = 0; i < width+5; i+= d) {
      vehicles1.add(new Vehicle(new PVector(i,0), random(0.1,0.4), 0.1, random(2)));
  }
  for (int i = 0; i < width+5; i+= d) {
      vehicles2.add(new Vehicle(new PVector(i,height), random(0.1,0.4), 0.1, random(2)));
  }
  for (int i = 0; i < width+5; i+= d) {
      vehicles3.add(new Vehicle(new PVector(0,i), random(0.1,0.4), 0.1, random(2)));
  }
  for (int i = 0; i < width+5; i+= d) {
      vehicles4.add(new Vehicle(new PVector(width,i), random(0.1,0.4), 0.1, random(2)));
  }  
}
